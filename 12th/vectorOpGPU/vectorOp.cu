...
__global__ void
vector_operation_kernel(int* output, int* data, int work_per_thrd, int size)
{
  for (int tid = threadIdx.x; tid < size; tid += blockIdx.x * blockDim.x)
...
main(int argc, char* argv[])
{
  ...
  grid_size = (data_size - 1) / block_size * work_per_thrd + 1;
