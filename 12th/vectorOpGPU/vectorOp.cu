...
__global__ void vector_operation_kernel_coalesced(int *output, int *data,
                                                  int size) {
...
int main(int argc, char *argv[]) {
  ...
  CUDA_CHECK_RETURN(cudaMemcpy(data_d, data_h, sizeof(int) * data_size,
                               cudaMemcpyHostToDevice));

  // vector_operation_kernel<<< grid_dime, block_dime >>>(output_d, data_d,
  // data_size, work_per_thread);
  vector_operation_kernel_coalesced<<<grid_dime, block_dime>>>(output_d, data_d,
                                                               data_size);
  ...
  CUDA_CHECK_RETURN(cudaMemcpy(device_output_h, output_d,
